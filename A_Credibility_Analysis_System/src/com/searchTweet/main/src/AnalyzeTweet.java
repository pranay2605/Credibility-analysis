...
double intensifier[] = new double[10]; // an array that stores the intensifiers
	...
	public double AnalyzeTweets(String tweet) {
		score = 0;
		this.Tweet = tweet;
		Tweet = SentimentAnalysis.replaceAbbv(Tweet);
		Tweet = SentimentAnalysis.removeURL(Tweet);
		Tweet = SentimentAnalysis.removeHash(Tweet);
		Tweet = SentimentAnalysis.removeDuplicates(Tweet);
		calculateEmoticons();
		Tweet = SentimentAnalysis.removeChars(Tweet);		
		calculateScore();
		return score;
	}
	...
	//	1 for intensifiers and 2 for negations; assuming there cannot be more than 80 words in a tweet
		...
		try {
			AspectDetection maps = new AspectDetection();		// create the maps that store the scores
